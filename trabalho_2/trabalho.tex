...
\begin{abstract}
Este trabalho apresenta uma implementação em C++ de um TAD de grafo para apoiar a escolha de um bairro para instalação de um galpão logístico. A solução lê os dados de vértices e arestas a partir de arquivos texto, calcula distâncias mínimas com os algoritmos de Dijkstra e Bellman-Ford, estima o custo operacional mensal e anual de cada candidato e escolhe o melhor bairro com base na expressão definida no enunciado do trabalho. Também foi incluída uma rotina de benchmark que grava em CSV as médias de tempo dos dois algoritmos após várias repetições.
\end{abstract}

\section{Introdução}

O problema proposto consiste em selecionar o melhor bairro para instalar um galpão logístico. Cada bairro possui um custo fixo de instalação, uma demanda estimada de mercadorias para cada mês do ano e conexões com bairros vizinhos por meio de arestas com custo de transporte.

Como o custo de entrega é proporcional à distância entre o bairro de origem e o bairro de destino, a solução depende do cálculo eficiente das menores distâncias entre o depósito e os demais bairros da cidade. Para isso, foram usados dois algoritmos clássicos de caminhos mínimos:
...
Os dois algoritmos devem encontrar as mesmas distâncias, mas possuem custos computacionais diferentes.
...
O grafo foi modelado como não direcionado, pois o enunciado descreve as conexões em termos de vizinhança entre bairros, com distância conhecida entre eles, sem explicitar direção.

Do ponto de vista de orientação a objetos, a modelagem buscou três propriedades principais. A primeira é o encapsulamento, já que os dados internos do grafo ficam protegidos dentro da classe \texttt{Grafo} e são manipulados apenas por seus métodos públicos. A segunda é a coesão, porque a classe concentra responsabilidades diretamente ligadas ao domínio do problema: armazenamento da estrutura, cálculo de caminhos mínimos, avaliação dos bairros e comparação de desempenho. A terceira é a delegação de responsabilidades, pois a regra de negócio da escolha do galpão e a produção dos resultados do benchmark foram colocadas no próprio TAD, enquanto o programa principal ficou restrito à leitura dos argumentos e à apresentação das saídas.
...
Essa organização foi adotada também como justificativa para o uso coerente de orientação a objetos no trabalho. Em vez de manter a lógica de decisão espalhada em funções externas, o comportamento central do problema passou a ser expresso por métodos do próprio objeto que representa o grafo da cidade. Assim, a estrutura de dados não apenas armazena informações, mas também conhece as operações necessárias para analisá-las dentro do contexto do domínio.
...
No programa principal, os vetores retornados pelos dois algoritmos são comparados. Caso os resultados sejam diferentes, a execução é encerrada, pois isso indicaria erro de implementação.

Para tornar a modelagem mais orientada a objetos, a análise de negócio também foi incorporada ao TAD por meio dos métodos:
...
Ao final, a decisão segue literalmente a expressão fornecida no enunciado:

\begin{lstlisting}
custoGalpao = custoInstalacao - custoOperacaoAnual
\end{lstlisting}

Mesmo sendo uma formulação econômica incomum, ela foi preservada sem alteração para manter aderência ao trabalho solicitado. Uma interpretação possível para essa expressão é tratá-la não como um custo total no sentido estrito, mas como um saldo entre o investimento fixo inicial e o impacto anual da operação. Nessa leitura, valores negativos indicam que o custo operacional anual superou o custo fixo de instalação, o que ajuda a explicar os resultados obtidos nos testes. Ainda assim, neste trabalho a fórmula foi aplicada literalmente, sem reinterpretar o critério de decisão além do que foi fornecido no enunciado.
...
Foram usadas duas instâncias de teste. A primeira possui 5 bairros, como exigido pelo enunciado. A segunda foi adicionada apenas para fortalecer a comparação experimental entre os algoritmos.
...
Pela regra do enunciado, o melhor bairro foi o bairro 5, pois apresentou o menor valor de \texttt{custoGalpao}.
